...
export function CollectionHeader({ collection }: CollectionHeaderProps) {
  ...
  // 슬러그 debounce
  useEffect(() => {
    if (slug === collection.slug) { setSlugStatus('idle'); return }
      ...
      {collection.isPublic && (
        <div className="space-y-1">
          ...
          {slugStatus === 'available' && (
            <p className="text-xs text-green-600" data-testid="slug-available-msg">{t('slugAvailable')}</p>
          )}
          {slugStatus === 'taken' && (
            <p className="text-xs text-destructive" data-testid="slug-taken-msg">{t('slugTaken')}</p>
          )}
