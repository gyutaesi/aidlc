...
function LinkBlock({
  ...
  return (
    ...
    >
      ...
      {content.description && (
        <p className="text-muted-foreground mt-1 line-clamp-2 text-sm">{content.description}</p>
      )}
...
export function PublicBlockRenderer({ blocks, collectionId, template }: PublicBlockRendererProps) {
  ...
  const containerClass =
    template === 'profile' ? 'grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3' : 'space-y-4'
        ...
        if (block.type === 'text') {
          ...
          return (
            <div
              ...
              data-testid={`public-block-text-${block.id}`}
            >
        ...
        if (block.type === 'image') {
          ...
          return (
            <div
              key={block.id}
              className="rounded-lg border p-4"
              data-testid={`public-block-image-${block.id}`}
            >
              <Image
              ...
              />
              {content.caption && (
                <p className="text-muted-foreground mt-2 text-center text-sm">{content.caption}</p>
              )}
            </div>
          )
        }
