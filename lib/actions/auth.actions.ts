...
import { SignUpSchema, SignInSchema, VerifySchema, ChangePasswordSchema } from '@/lib/schemas/auth.schema'
...
export async function confirmSignUpAction(formData: FormData): Promise<ActionResult> {
  ...
  try {
    ...
    logger.error('ConfirmSignUp failed', { error: error instanceof Error ? error.message : 'Unknown' })
...
export async function changePasswordAction(formData: FormData): Promise<ActionResult> {
  ...
  try {
    await authService.changePassword(accessToken, parsed.data.currentPassword, parsed.data.newPassword)
    return { success: true }
  } catch (error) {
    logger.error('ChangePassword failed', { error: error instanceof Error ? error.message : 'Unknown' })
