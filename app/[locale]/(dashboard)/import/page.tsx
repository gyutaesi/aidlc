...
export default function ImportPage() {
  ...
  async function handleImport() {
    ...
    if (result.success) {
      toast.success(
        t('success', { imported: result.imported ?? 0, failed: result.failed ?? 0 })
      )
      setFile(null)
    } else {
  ...
  return (
    <div className="mx-auto max-w-lg p-6" data-testid="import-page">
      ...
      >
        ...
        <p className="mt-1 text-xs text-muted-foreground">{t('fileType')}</p>
        ...
        <label className="mt-4 inline-block cursor-pointer">
          ...
          />
          <span className="rounded-md border px-3 py-1.5 text-sm hover:bg-accent">
            파일 선택
          </span>
        </label>
      </div>
